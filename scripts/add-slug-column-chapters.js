...
async function addSlugColumnToChapters() {
  try {
    ...
    const { error: addColumnError } = await supabase.rpc('exec_sql', {
      sql: 'ALTER TABLE public.chapters ADD COLUMN IF NOT EXISTS slug text;'
    });
    ...
    const { error: commentError } = await supabase.rpc('exec_sql', {
      sql: "COMMENT ON COLUMN public.chapters.slug IS 'URL-friendly identifier generated from the chapter title';"
    });
    ...
    const { error: indexError } = await supabase.rpc('exec_sql', {
      sql: 'CREATE INDEX IF NOT EXISTS idx_chapters_slug ON public.chapters(slug);'
    });
...
END $$;`
    ...
    if (error) {
      ...
      if (error.message.includes("slug")) {
        ...
        console.log('ALTER TABLE public.chapters ADD COLUMN IF NOT EXISTS slug text;');
        console.log("COMMENT ON COLUMN public.chapters.slug IS 'URL-friendly identifier generated from the chapter title';");
        console.log('CREATE INDEX IF NOT EXISTS idx_chapters_slug ON public.chapters(slug);');
        console.log(`DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
END $$;`);
      ...
      console.log('✓ Successfully verified access to chapters table with slug column');
